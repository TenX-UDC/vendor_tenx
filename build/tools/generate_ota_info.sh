...
if [ "$1" ]; then
    ...
    if [ -f $file_path ]; then
        ...
        name=$(grep ro\.tenx\.device\.name ${build_prop} | cut -d= -f2);
        codename=$(grep ro\.tenx\.device\= ${build_prop} | cut -d= -f2);
        version=$(grep ro\.tenx\.version ${build_prop} | cut -d= -f2);
        romtype=$(grep ro\.tenx\.releasetype ${build_prop} | cut -d= -f2);
        variant=$(grep ro\.tenx\.build\.variant ${build_prop} | cut -d= -f2);
        variant_lower=$(echo ${variant} | tr '[:upper:]' '[:lower:]');
        maintainer_name=$(grep ro\.tenx\.maintainer\= ${build_prop} | cut -d= -f2);
        maintainer_username=$(grep ro\.tenx\.maintainer\.username ${build_prop} | cut -d= -f2);
        size=$(stat -c%s $file_path);
        datetime=$(grep ro\.build\.date\.utc ${build_prop} | cut -d= -f2);
        filehash=$(md5sum $file_path | awk '{ print $1 }');
        id=$(sha256sum $file_path | awk '{ print $1 }');
        url="https://sourceforge.net/projects/tenx-os/files/${device_name}/${file_name}/download";
        group=$(grep ro\.tenx\.group\.url ${build_prop} | cut -d= -f2);
        echo "{" > $file_path.json
